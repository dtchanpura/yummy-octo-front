(function() {
    ...
    function RemoteController($location, $cookies, FlashService, ConnectionService) {
        ...
        function action(act, vol_param) {
            ConnectionService.Action($cookies.get('base_url'), {
        ...
        function queue_f() {
            ...
            ConnectionService.Queue($cookies.get('base_url'), {
                token: $cookies.get('token'),
                path: pth
            }).then(function(response) {
                if (response !== undefined && response.ok) {
        ...
        function startDaemon() {
            ConnectionService.StartDaemon($cookies.get('base_url'), {
                token: $cookies.get('token')
            }).then(function(response) {
                if (response !== undefined && response.ok) {
                    updateUI(response);
        ...
        function quitSession() {
            ConnectionService.QuitSession($cookies.get('base_url'), {
                token: $cookies.get('token')
            }).then(function(response) {
                if (response !== undefined && response.ok) {
                    vm.check_queue = "remove-circle";
